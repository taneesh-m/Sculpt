-- Safe database schema that checks for existing objects before creating them
...
BEGIN
    ...
    -- Exercises policies
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'exercises' AND policyname = 'Users can view exercises from own workouts') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'exercises' AND policyname = 'Users can insert exercises to own workouts') THEN
        CREATE POLICY "Users can insert exercises to own workouts" ON exercises FOR INSERT WITH CHECK (
            EXISTS (SELECT 1 FROM workouts WHERE workouts.id = exercises.workout_id AND workouts.user_id = auth.uid())
        );
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'exercises' AND policyname = 'Users can update exercises from own workouts') THEN
        CREATE POLICY "Users can update exercises from own workouts" ON exercises FOR UPDATE USING (
            EXISTS (SELECT 1 FROM workouts WHERE workouts.id = exercises.workout_id AND workouts.user_id = auth.uid())
        );
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'exercises' AND policyname = 'Users can delete exercises from own workouts') THEN
    ...
    -- Diet logs policies
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'diet_logs' AND policyname = 'Users can view own diet logs') THEN
        CREATE POLICY "Users can view own diet logs" ON diet_logs FOR SELECT USING (auth.uid() = user_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'diet_logs' AND policyname = 'Users can insert own diet logs') THEN
        CREATE POLICY "Users can insert own diet logs" ON diet_logs FOR INSERT WITH CHECK (auth.uid() = user_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'diet_logs' AND policyname = 'Users can update own diet logs') THEN
        CREATE POLICY "Users can update own diet logs" ON diet_logs FOR UPDATE USING (auth.uid() = user_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'diet_logs' AND policyname = 'Users can delete own diet logs') THEN
        CREATE POLICY "Users can delete own diet logs" ON diet_logs FOR DELETE USING (auth.uid() = user_id);
    END IF;

    -- Nutrition goals policies
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'nutrition_goals' AND policyname = 'Users can view own nutrition goals') THEN
        CREATE POLICY "Users can view own nutrition goals" ON nutrition_goals FOR SELECT USING (auth.uid() = user_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'nutrition_goals' AND policyname = 'Users can insert own nutrition goals') THEN
        CREATE POLICY "Users can insert own nutrition goals" ON nutrition_goals FOR INSERT WITH CHECK (auth.uid() = user_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'nutrition_goals' AND policyname = 'Users can update own nutrition goals') THEN
        CREATE POLICY "Users can update own nutrition goals" ON nutrition_goals FOR UPDATE USING (auth.uid() = user_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'nutrition_goals' AND policyname = 'Users can delete own nutrition goals') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'progress_tracking' AND policyname = 'Users can delete own progress') THEN
